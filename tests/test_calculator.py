...
def test_calculator_division_by_zero(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Cannot divide by zero, please enter a non-zero denominator" in captured.out
...
def test_calculator_unexpected_exception(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "An error has occurred: Mock exception during execution" in captured.out
